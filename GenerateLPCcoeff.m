function [LPCpoles, G, FFerr, formantFreqs] = GenerateLPCcoeff(...
    WindowedAudio,modelOrder,nFrames,WinL,FrameL,PreEmphAlpha,Fs,SnapInd,LiftLowQueRecp,LiftHighQueRecp,file,pMode,diag)
%MyLPC(pm,data,diag)
%diag: true to enable lifter output plot


%pre-allocate
formantFreqs(1:modelOrder,1:nFrames) = nan;
CLift = nan(WinL,nFrames);
Cwind = CLift;
G = nan(nFrames,1);
LPCpoles = nan(modelOrder+1,nFrames);
FFerr = CLift;
for i = 1:nFrames
%% remove glottal impulses (or vocal tract) via cepstral liftering
if ~all(WindowedAudio(:,i) ==0)
  % pre-emphasize speech for LPC coding +6dB per octave
...
%% find pole center frequencies
   ...
   % to find the location of a root, find its angle w.r.t. w = 0 using atan2.
...
if diag
  ...
  title({['Poles/zeros of LPC Vocal Tract model @ Time= ',num2str(SnapInd*FrameL/Fs),' sec.']; txt})
  ...
  q=(StartAx:StopAx)/Fs;
  figure
  stem(q,CLift(StartAx:StopAx,SnapInd),'displayname','Liftered Cepstrum')
  hold on
  stem(q,Cwind(StartAx:StopAx,SnapInd),'r','displayname','un-liftered cepstrum')
  title({['Liftered Cepstrum into LPC Vocal Tract predictor at Time =  ',num2str(SnapInd*FrameL/Fs), ' sec.'], txt})
  xlabel('Quefrency [s]')
  ylabel('Cepstral Amplitude (relative)')
  legend('show')
end
